...
@section('subtitle', 'Riwayat peminjaman aset klub')
...
@section('actions')
    @if (in_array(auth()->user()->role, config('sh3.inventory_manage_roles'), true))
        <a href="{{ route('admin.inventory.loans.create') }}" class="btn btn-primary btn-sm">
...
<div class="card">
    <div class="card-header">
        <div class="flex w-full flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <form action="{{ route('admin.inventory.loans.index') }}" method="GET" class="relative sm:w-72" role="search">
                    ...
                    <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Cari peminjam, tujuan, aset..." class="search-input" aria-label="Cari peminjaman">
                </form>

                <form action="{{ route('admin.inventory.loans.index') }}" method="GET" class="flex flex-wrap items-center gap-2">
                    ...
                    <div class="relative">
                        <select name="status" onchange="this.form.submit()" class="form-select py-2 text-sm" aria-label="Filter status">
                            ...
                            @foreach (\App\Models\InventoryLoan::STATUSES as $status)
                    ...
                    @if (!empty($filters['search']) || !empty($filters['status']) || !empty($filters['overdue']))
                        <a href="{{ route('admin.inventory.loans.index') }}" class="filter-chip" aria-label="Reset filter">
            ...
            <span class="badge badge-secondary shrink-0">{{ $loans->total() }} peminjaman</span>
        </div>
    </div>
            ...
            <tbody>
                @forelse ($loans as $loan)
                    <tr>
                        ...
                        <td>
                            <span class="font-mono text-sm font-medium text-slate-900 dark:text-slate-100">{{ $loan->item?->asset_code }}</span>
                            <span class="block text-xs text-slate-500 dark:text-slate-400">{{ $loan->item?->name }}</span>
                        </td>
                        ...
                        <td colspan="8">
                            <div class="empty-state">
                                ...
                                @if (!empty($filters['search']) || !empty($filters['status']) || !empty($filters['overdue']))
                                    ...
                                    <p class="empty-state-text">Tidak ditemukan peminjaman yang cocok dengan filter Anda.</p>
                                    <a href="{{ route('admin.inventory.loans.index') }}" class="btn btn-secondary btn-sm mt-4">Reset Filter</a>
                                @else
                                    <p class="empty-state-title">Belum ada peminjaman</p>
                                    <p class="empty-state-text">Buat peminjaman pertama untuk mulai mencatat peminjaman aset klub.</p>
                                    @if (in_array(auth()->user()->role, config('sh3.inventory_manage_roles'), true))
                                        <a href="{{ route('admin.inventory.loans.create') }}" class="btn btn-primary btn-sm mt-4">
    ...
    @include('includes.pagination', ['items' => $loans, 'label' => 'peminjaman'])
</div>
@endsection
