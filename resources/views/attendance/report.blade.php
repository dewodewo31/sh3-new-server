...
@section('subtitle', 'Lihat attendance berdasarkan event')

@section('actions')
    <a href="{{ route('admin.attendance.scan') }}" class="btn btn-primary btn-sm">
        ...
        <div class="card-body">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-slate-100 truncate">{{ $event->title }}</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">{{ $event->start_date->format('d/m/Y') }}</p>
                </div>
                <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 shrink-0 ml-4">
