...
@section('title', 'Tambah Anggaran Event')
@section('subtitle', 'Buat anggaran per event dan aktivitas')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-header-title">Tambah Anggaran Event</h3>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.event-budgets.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="form-group">
                    ...
                    <select name="activity_id" required class="form-select @error('activity_id') error @enderror">
                        ...
                        @foreach($activities as $id => $name)
                            <option value="{{ $id }}" {{ old('activity_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2 form-group">
                    ...
                    <textarea name="notes" rows="2" class="form-textarea @error('notes') error @enderror">{{ old('notes') }}</textarea>
