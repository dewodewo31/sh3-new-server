...
<body>
    <div class="header">
        ...
        <div class="meta">Laporan Piutang</div>
        @if($event)
    ...
    <h2>Ringkasan Piutang</h2>

    <div class="summary">
        <div>Komitmen: <b>{{ number_format($summary['committed'], 0, ',', '.') }}</b></div>
        <div class="pos">Diterima: <b>{{ number_format($summary['received'], 0, ',', '.') }}</b></div>
        <div class="{{ $summary['outstanding'] >= 0 ? 'pos' : 'neg' }}">Sisa Piutang: <b>{{ number_format($summary['outstanding'], 0, ',', '.') }}</b></div>
    </div>
</body>
</html>
