...
@section('subtitle', 'Komitmen, diterima, dan sisa piutang')

@section('content')
<div class="flex flex-wrap items-center gap-2 mb-5">
    <a href="{{ route('admin.bookkeepings.reports') }}" class="btn btn-secondary btn-sm">Dashboard</a>
    <a href="{{ route('admin.bookkeepings.receivables') }}" class="btn btn-primary btn-sm">Piutang</a>
...
@php
    ...
    $rcsv = route('admin.bookkeepings.export-receivables', array_merge(request()->only($filterKeys), ['format' => 'csv']));
    $rxlsx = route('admin.bookkeepings.export-receivables', array_merge(request()->only($filterKeys), ['format' => 'xlsx']));
    $rpdf = route('admin.bookkeepings.export-receivables', array_merge(request()->only($filterKeys), ['format' => 'pdf']));
@endphp
<div class="flex flex-wrap items-center gap-2 mb-5">
    <span class="text-sm text-slate-500 dark:text-slate-400">Ekspor:</span>
    <a href="{{ $rcsv }}" class="btn btn-secondary btn-sm">CSV</a>
    <a href="{{ $rxlsx }}" class="btn btn-secondary btn-sm">XLSX</a>
    <a href="{{ $rpdf }}" class="btn btn-secondary btn-sm">PDF</a>
</div>
...
<div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
    ...
    <div class="stat-card">
        <div class="flex items-start justify-between">
            <div>
                <p class="text-sm font-medium text-slate-500 dark:text-slate-400">Diterima</p>
                <p class="mt-2 text-3xl font-bold tracking-tight text-emerald-600 dark:text-emerald-400">Rp {{ number_format($received, 0, ',', '.') }}</p>
            </div>
    ...
    <div class="stat-card">
        <div class="flex items-start justify-between">
            <div>
                <p class="text-sm font-medium text-slate-500 dark:text-slate-400">Sisa Piutang</p>
